;; Text, like this, that starts with semi-colons is comment
;;
...
to setup
  ...
  ;; TASK 2-1
  ;; make 'num-targets' of patches red - the target
  ask n-of num-targets patches with [pcolor = black] [set pcolor red]
  ...
  create-myagents 1 [
    ...
    ;; move to a random black patch as starting patch
    move-to one-of patches with [pcolor = black]
  ]

  ;; this initialises the simulation time system and graphs
  reset-ticks
end

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Next we have the procedure to progress the simulation one time step
;; This is executed when one presses the "step" button
...
to go
  ...
  ask myagents [
    ...
    if not finished? [
      ...
      ;; TASK 2-1
      if random-float 1 < turn-left-prob [lt 90]
...
This model also introduce how to create basic GUI components such as buttons and sliders.
...
1. add a button to go 10 steps forward (using "repeat 10 [go]")
1. add a button to manually turn the agent right (using "rt 90")
1. add a button to "forever" do "go"
1. add a slider for number of targets and add code to make this number of patches red
1. try to change the "if" commands within the "go" procedure and see what happens
1. add new "if" rules, for example to with a certain probability to turn left (using "if random-float 1 < 0.05 [.....]")
1. add a command within "setup" to place the agent at a random position at the start (using "setxy", "random max-xcor" and random max-ycor")
...
Modified by Shih-Fen Cheng, 2016, 2017
@#$#@#$#@
default
true
0
...
@#$#@#$#@
<experiments>
  <experiment name="Task 2-2" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="15000"/>
    <steppedValueSet variable="num-targets" first="1" step="1" last="5"/>
    <steppedValueSet variable="turn-left-prob" first="0.1" step="0.1" last="0.9"/>
    <enumeratedValueSet variable="num-obstacles">
      <value value="200"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
